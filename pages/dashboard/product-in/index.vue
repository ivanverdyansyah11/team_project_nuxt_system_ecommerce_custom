...
const updateProductIn = handleSubmit( async (values) => {
  values.date = new Date(values.date).toISOString();
  await productInStore.updateProductIn(values, productInData.value.id);
  if (productInStore.status_code === 200) {
    Cookies.set('alert-message', 'Successfully update product in');
    Cookies.set('alert-page', 'Product In');
    alertMessage = useCookie('alert-message')
    alertPage = useCookie('alert-page')
    setValues({
...
const confirmDeleteProductIn = async () => {
  if (productInData.value) {
    await productInStore.deleteProductIn(productInData.value.id);
    if (productInStore.status_code === 200) {
      Cookies.set('alert-message', 'Successfully deleted product in');
      Cookies.set('alert-page', 'Product In');
      alertMessage = useCookie('alert-message')
      alertPage = useCookie('alert-page')
    }
...
<template>
  <div class="content container mt-4">
            ...
            <table class="table" style="width:100%">
              ...
              <tr v-if="productInLength > 0" v-for="(productIn, index) in productInStore.productInAll" :key="index">
                ...
                <td class="d-flex justify-content-end gap-1 table-mobile" style="width: 200px;">
                  ...
                  <button type="button" data-bs-toggle="modal" data-bs-target="#updateModal" @click="productInDetail(productIn)"
                  ...
                  <button type="button"
                          ...
                          data-bs-toggle="modal" data-bs-target="#deleteModal" @click="productInData = productIn">
    ...
    <div class="modal fade" id="updateModal" tabindex="-1" aria-labelledby="updateModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <form @submit.prevent="updateProductIn" class="form w-100">
            <div class="modal-header d-flex align-items-center justify-content-between">
              <h1 class="modal-title fs-5" id="updateModalLabel">Edit Product In</h1>
              <button type="button" data-bs-dismiss="modal" aria-label="Close" @click="removeProductInDetail()">
                <i class="fa-solid fa-xmark"></i>
              </button>
            </div>
            ...
            <div class="modal-footer">
              <button type="button" class="button-reverse" data-bs-dismiss="modal" @click="removeProductInDetail()">Cancel Edit</button>
    ...
    <div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header d-flex align-items-center justify-content-between">
            <h1 class="modal-title fs-5" id="deleteModalLabel">Delete Product In</h1>
            <button type="button" data-bs-dismiss="modal" aria-label="Close">
              <i class="fa-solid fa-xmark"></i>
            </button>
          </div>
          <div class="modal-body">
            <p style="font-size: 0.913rem">Are you sure want to delete this product in?</p>
          </div>
          <div class="modal-footer">
            <button type="button" class="button-reverse" data-bs-dismiss="modal">Cancel Delete</button>
            <button type="button" class="button-primary-small" @click="confirmDeleteProductIn" data-bs-dismiss="modal">Delete Product In</button>
          </div>
        </div>
      </div>
    </div>
